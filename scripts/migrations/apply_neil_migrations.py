#!/usr/bin/env python3
"""
Apply Neil migration SQL files with safe default dry-run mode.

Usage:
  python3 scripts/migrations/apply_neil_migrations.py
  python3 scripts/migrations/apply_neil_migrations.py --execute
"""
...
MIGRATION_FILES = [
    ROOT / "scripts" / "database" / "migrations" / "create_neil_stock_pool.sql",
    ROOT / "scripts" / "database" / "migrations" / "create_neil_turtle_signals.sql",
]

SNAPSHOT_PATH = ROOT / "logs" / "neil_migration_snapshot.json"
...
def build_snapshot(conn: sqlite3.Connection) -> dict:
    return {
        ...
        "indexes": {
            "ux_neil_pool_biz_key": index_exists(conn, "ux_neil_pool_biz_key"),
            "idx_neil_pool_code": index_exists(conn, "idx_neil_pool_code"),
            "idx_neil_pool_active": index_exists(conn, "idx_neil_pool_active"),
            "idx_neil_pool_import_batch": index_exists(conn, "idx_neil_pool_import_batch"),
            "ux_neil_sig_biz_key": index_exists(conn, "ux_neil_sig_biz_key"),
            "idx_neil_sig_code_date": index_exists(conn, "idx_neil_sig_code_date"),
            "idx_neil_sig_task_date": index_exists(conn, "idx_neil_sig_task_date"),
            "idx_neil_sig_new_entry_date": index_exists(conn, "idx_neil_sig_new_entry_date"),
            "idx_neil_sig_pool": index_exists(conn, "idx_neil_sig_pool"),
        },
    }
...
def backup_db(db_path: Path) -> Path:
    ...
    target = backup_dir / f"stock_data_before_neil_migration_{timestamp}.db"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply Neil migrations safely")
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    try:
        ...
        # Guardrail: old table row counts should not change in this migration.
