#!/usr/bin/env python3
"""
Import Neil stock pool from Excel into neil_stock_pool.

Default mode is dry-run (no DB writes).

Usage:
  python3 scripts/upload_neil_stock_pool.py --file /Users/mac/Downloads/neil.xls
  python3 scripts/upload_neil_stock_pool.py --file /Users/mac/Downloads/neil.xls --execute
  python3 scripts/upload_neil_stock_pool.py --file /Users/mac/Downloads/neil.xls --execute --deactivate-missing
"""
...
INDEX_PREFIX_EXCLUDES = ("399",)  # 深证指数
    ...
    try:
        ...
        if deactivate_missing:
            ...
            if imported_codes:
                ...
                """
                cur.execute(sql, imported_codes)
            else:
                cur.execute(
                    """
                    UPDATE neil_stock_pool
        ...
        # Always deactivate manual excluded codes to keep active pool clean.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import neil.xls into neil_stock_pool")
    ...
    parser.add_argument(
        "--deactivate-missing",
        action="store_true",
        help="Set is_active=0 for stocks missing in current file (only with --execute)",
    )
    parser.add_argument(
        "--exclude-codes",
        default=",".join(DEFAULT_MANUAL_EXCLUDE_CODES),
        help="Comma-separated stock codes to exclude from import (default: 000680)",
    )
    ...
    report_path = Path(args.report).expanduser().resolve() if args.report else (
        ROOT / "logs" / f"neil_import_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
