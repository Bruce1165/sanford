"""
eight_wave_stock_picker.py - 基于八段论方法论推荐未来21-34天内有上涨20%的潜力股
...
class EightWaveStockPicker:
    def __init__(self, db_path: str, lookback_days: int = 60):
        self.db_path = db_path
        self.lookback_days = lookback_days

    def get_all_stocks(self) -> list:
        """获取指定领域的股票代码 - 使用精确行业匹配"""
        # 指定精确领域关键词（移除宽泛词汇）
        target_categories = {
            '人工智能': ['人工智能', 'AIGC', '大语言模型', '生成式AI', '大模型'],
            '东数西算': ['东数西算', '算力中心'],
            '创新药': ['创新药', 'PD-1', 'PD-L1', 'ADC', '生物药'],
            '半导体': ['半导体', '集成电路', '芯片', '光刻胶', '光掩膜', '化合物半导体'],
            '储能': ['储能', '电池', '钠电池', '固态电池'],
            '绿色电力': ['光伏', '风电', '太阳能', '风能'],
            '国产算力': ['国产GPU', '国产CPU', '信创', '自主可控']
        }
        ...
        for category, keywords in target_categories.items():
            for keyword in keywords:
                industry_filters.append(f"(industry LIKE '%{keyword}%')")
    ...
    def analyze_stock_wave_phase(self, df: pd.DataFrame) -> dict:
        ...
        # 布林带位置
        bollinger_upper_touch = latest['close'] > latest['bollinger_upper'] * 0.95
        bollinger_lower_touch = latest['close'] < latest['bollinger_lower'] * 1.05
...
def main():
    ...
    picker = EightWaveStockPicker(db_path)
    ...
    if top_stocks:
        ...
        print("2. 阶段评分：基于均线排列、MACD、RSI、成交量等指标判断个股所处阶段")
        print("3. 技术评分：均线多头、MACD金叉、RSI合理、成交量放大等")
        print("4. 基本面评分：ROE、PE等指标")
        print("5. 综合评分 = 阶段评分 + 技术评分 + 基本面评分")
        print("6. 仅基于DB现有数据，未包含宏观经济、政策、资金流向等外部数据")
        print("=" * 100)
