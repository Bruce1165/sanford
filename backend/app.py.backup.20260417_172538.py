...
def data_health():
    ...
    # Get last data date and active stocks count
    last_date = None
    last_date_obj = None
    active_stocks = 0
    status = 'healthy'
    ...
    if stock_db_exists:
        ...
        # Get active stocks count (non-delisted stocks)
        cursor.execute(
            'SELECT COUNT(*) FROM stocks WHERE is_delisted = 0'
        )
    ...
    if not stock_db_exists:
        ...
        status = 'warning'
    elif last_date_obj and last_date_obj < (date.today() - timedelta(days=2)):
        status = 'warning'
    else:
        status = 'healthy'
    ...
    return jsonify({
        'db_health': {
            'timestamp': last_date if last_date else None,
            'status': status,
            'checks': {
                'stock_pool': {
